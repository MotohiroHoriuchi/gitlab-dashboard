...
![4つのビューを切り替える様子](docs/overview.gif)

日々の絞り込み、4つのビューの使い分け、会議での見せ方は、画面付きの **[操作マニュアル（HTML）](docs/manual.html)** にまとめています。
...
> 遅れている予定は赤いハッチで一目瞭然です。凡例の曜日チップで非稼働曜日を非表示にできます（既定: 土日。ブラウザに保存され再訪時も維持）。

![カレンダーのホバー詳細](docs/calendar-tooltip.png)

---

![★をクリックすると大事な仕事が光る](docs/checkpoint-glow.gif)

> 凡例の ★をクリックしてみましょう。大事な仕事は光って教えてくれます。

![フィルタとビューだけを全画面に](docs/fullscreen.gif)

> 「⛶ 全画面」で、フィルタと今のビューだけを画面いっぱいに。会議で映すときや、じっくり眺めたいときに。Esc で戻ります。



### 4. ロードマップ

![ロードマップ](docs/roadmap.png)
...
- **カレンダー**：マイルストーン／イシューを月・2週タイムラインで表示
  ...
  - 週あたりの表示レーンに上限（月＝3 / 2週＝6、全画面時は倍）を設け、超過分は日別 **「+N 件」チップ**に集約。
...
- **ロードマップ**：1 マイルストーン 1 行で、開始→期限のバー・本日線・未完イシューの締切 tick（超過は赤、★ はチェックポイント）を表示。
  近い締切は個別ラベル、先の分は **「+N」チップ**に集約。左カラムに進捗率・残/超過・ミニ・バーンダウン。
...
- **全画面モード**：「⛶ 全画面」でフィルタバーと今のビューだけをビューポート全面に。**Esc** で解除。
  カレンダーは全画面時に表示レーン上限が倍（月 3→6 / 2週 6→12）
- **可読性**：カレンダー／ロードマップのタイポグラフィを 4 段階スケールに整理して底上げ、小さい文字のコントラストを強化
...
`MOCK_GITLAB=1` のときは `lib/devMock.ts` のサンプル（4ビューを一通り描けるよう滞留・分布・納期予実・マイルストーンを含む）で描画する。GitLab 認証は不要。
