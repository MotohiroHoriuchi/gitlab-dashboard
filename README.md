...
- **カレンダー** — マイルストーン／イシューを月・2週タイムラインで表示
  ...
  - バーにホバーで詳細ツールチップ（#ID・ラベル・担当者・状態・期間）
...
- GitLab の Issue は複数ラベルを持てるが、本ダッシュボードは元デザインに合わせ
  **先頭ラベル（GitLab が返す配列の先頭）を代表ラベル**として 1 件に集約して扱う。
  **ラベル絞り込み・ドロップダウンの選択肢・分布のラベル別集計はすべてこの代表ラベルで判定する**
  （＝複数ラベルのうち先頭以外では絞り込みに一致せず、どのイシューでも先頭に来ないラベルは選択肢に出ない）。
  例外として、カレンダーの ★ チェックポイント判定だけは全ラベルを見る。
