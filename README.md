...
![ビューを切り替える様子](docs/overview.gif)

日々の絞り込み、5つのビューの使い分け、会議での見せ方は、画面付きの **[操作マニュアル（HTML）](docs/manual.html)** にまとめています。
...
```sh
npm install
npm run dev:mock              # → http://localhost:48273
```
...
- **URL 状態保持**：タブ、絞り込み、並べ替え、分布の集計軸、カレンダーの表示期間を URL パラメータへ自動反映。
  再読み込み後も同じ画面を復元でき、ブックマークや URL 共有にも利用できる
- **並べ替え**：長引き順 / 新しい順 / 古い順
